            ...
            if search_option == "Search keywords":
                ...
                if search_term:
                    with st.expander('Click to expand', expanded=True):
                        ...
                        query = '|'.join(filters)  # Construct the query with all filters

                        df_csv = pd.read_csv('all_items.csv')

                        filtered_df = df_csv[
                            (df_csv['Title'].str.contains(query, case=False, na=False, regex=True)) |
                            (df_csv['FirstName2'].str.contains(query, case=False, na=False, regex=True))
                        ]
