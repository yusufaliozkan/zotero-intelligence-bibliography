    ...
    with tab1:
        ...
        with col1:
            ...
            with st.expander('Items by country (click to expand)', expanded=False):
                ...
                options_with_counts = [f"{country} ({country_counts.get(country, 0)})" if country != '' and country_counts.get(country, 0) > 0 else country for country in unique_countries]
                ...
                fig = px.bar(x=country_pub_counts.index, y=country_pub_counts.values)  # Swap x and y for vertical bars
                fig.update_layout(title='Number of Publications by Country', xaxis_title='Country', yaxis_title='Number of Publications')
                st.plotly_chart(fig)
