...
with st.spinner('Retrieving data & updating dashboard...'):
    ...
    with tab1:
        ...
        with col1:
            ...
            df_countries_chart['Date published'] = pd.to_datetime(df_countries_chart['Date published'],utc=True, errors='coerce').dt.tz_convert('Europe/London')
            df_countries_chart['Date published'] = df_countries_chart['Date published'].dt.strftime('%Y-%m-%d')
            df_countries_chart['Date published'] = df_countries_chart['Date published'].fillna('')
            df_countries_chart['No date flag'] = df_countries_chart['Date published'].isnull().astype(np.uint8)
            df_countries_chart = df_countries_chart.sort_values(by=['No date flag', 'Date published'], ascending=[True, True])
            df_countries_chart = df_countries_chart.sort_values(by=['Date published'], ascending=False)
            df_countries_chart = df_countries_chart.reset_index(drop=True)
