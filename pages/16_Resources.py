        ...
        if prog_type == 'Government institutions':
            ...
            with st.expander(f"{prog_type} ({len(type_programs)})"):
                if num_unique_countries != 1:
                    ...
                    selected_country = st.multiselect('Filter by country:', countries_sorted, default=countries_sorted, format_func=lambda x: country_counts_dict[x])
                    if selected_country:
                        ...
                        type_programs = df[df['Type'] == prog_type]  # Show all programs if no country is selected
                        st.write(f'**{len(type_programs)} {prog_type} found in {num_unique_countries} countries**')

                for country in type_programs['Country'].unique():
                    if num_unique_countries == 1 or (selected_country and country in selected_country):
